...
export class NgSelectComponent implements OnDestroy, OnChanges, OnInit, AfterViewInit, ControlValueAccessor {
    ...
    // 是使用了 @HostBinding 装饰器，它是 Angular 提供的一种方式，用于动态地为宿主元素绑定 HTML 属性或者 CSS 类。
    ...
    /**
     * 
     * 绑定方法到宿主元素 'keydown'事件
     * @param $event 接收一个 'KeyboardEvent'类型的参数 `$event`,表示键盘事件对象。
     * @returns 
     */
    ...
    handleKeyDown($event: KeyboardEvent) {
        ...
        if (keyCode) {
            ...
            //如果按键不是特殊功能键（没有对应的键码），并且按键是一个单个字符，
            //则将该字符转换为小写并发送给 _keyPress$ 主题（是一个 RxJS 的 Subject）
    ...
    private _handleKeyPresses() {
        ...
        // 监听按键  将输入的按键字符拼接成一个字符串，然后在itemList中查找与该字符串匹配的项。
        this._keyPress$
            .pipe(takeUntil(this._destroy$),
                tap(letter => this._pressedKeys.push(letter)),
                // 防抖函数 确保按键没有更没有在这段时间更新才会继续
                debounceTime(200),
                // 过滤掉按键数量为零的情况
