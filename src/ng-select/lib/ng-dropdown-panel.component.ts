...
export class NgDropdownPanelComponent implements OnInit, OnChanges, OnDestroy {
  ...
  /**
   作用是监听组件中指定的滚动元素的滚动事件，并在滚动时获取滚动距离（scrollTop）。
   使用 auditTime 以特定的时间间隔发出最新的值，同时在 Angular 变更检测之外运行这段代码，以提高性能并避免不必要的变更检测。
   */
  private _handleScroll() {
    // this._zone.runOutsideAngula 表示在angular变更检测外运行，以避免触发不必要的变更检测 提高性能。
    this._zone.runOutsideAngular(() => {
      // fromEvent 创建一个可观察对象  this.scrollElementRef.nativeElement是模板组件滚动条的引用
      fromEvent(this.scrollElementRef.nativeElement, "scroll")
        // 用于将多个操作费串联  takeUntil在销毁标志this._destroy$发出值时终止订阅，防止内存泄漏
        // auditTime操作符，指定间隔后发出最新的值 SCROLL_SCHEDULER为一个调度器
        .pipe(takeUntil(this._destroy$), auditTime(0, SCROLL_SCHEDULER))
        // 订阅从'fromEvent'中得到的滚动事件流
        .subscribe((e: { path; composedPath; target }) => {
          const path = e.path || (e.composedPath && e.composedPath());
        //   如果没有找到路径信息或者路径信息为空 直接使用e.target.scrollTop 获取滚动距离
          const scrollTop =
...
// 检测滚动条是不是滚动到了底部
  private _fireScrollToEnd(scrollTop: number) {
    // 如果scrollTop 滚动条已经在底部 或者在顶部直接返回 
    ...
    const padding = this.virtualScroll
      ...
      // 它计算滚动位置是否已经接近或达到底部。它通过比较 scrollTop（滚动位置）加上 _dropdown.clientHeight（下拉框的高度）
      //是否大于或等于 padding.clientHeight - 1（所选容器的高度减去一个小的边界值）来判断是否接近底部。
